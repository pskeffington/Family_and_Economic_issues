...
\end{adjustbox}
\begin{minipage}{0.95\textwidth}
\footnotesize \textit{Notes:} FE = fixed effects. LPM = linear probability model. HR = hazard ratio. Poisson estimates are log-count coefficients. For the Cox PH row, the estimate is reported as a hazard ratio and the standard error is for the log-hazard coefficient. Standard errors are robust where available and clustered at the respondent level in panel specifications. All estimates are interpreted as associations rather than causal effects. The financial-literacy coefficient is not statistically different from zero in the primary Poisson model.
\end{minipage}
\end{table}
...
The Cox proportional hazards model provides a timing-based check for the birth-control education result. The hazard ratio below one indicates that respondents exposed to adolescent birth-control education had a lower observed hazard of first birth at a given time. This finding is consistent with later entry into parenthood and complements the completed-fertility result. The timing and completed-fertility models therefore point in the same broad direction for reproductive education, although neither model establishes causality.
...
Cox proportional hazards model & Assess timing of first birth & Birth-control education is associated with a lower hazard of first birth, consistent with later entry into parenthood. \\
...
Adolescent birth-control education was negatively associated with completed fertility and was also associated with later entry into parenthood in the Cox proportional hazards model. This pattern is consistent with a life-course interpretation in which reproductive knowledge is linked to greater control over pregnancy timing, delayed first birth, and smaller completed family size. The finding aligns with prior research showing that contraception and reproductive control can alter schooling, work, marriage, and fertility trajectories \citep{goldin2002,bailey2010,kearney2009,lindberg2016,myers2017}. At the same time, the measure used here captures reported exposure to birth-control education, not curriculum quality, contraceptive access, or contraceptive behavior.
